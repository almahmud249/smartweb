...
class Helpers
{
    ...
    public static function set_data_format($data)
    {
        try {
            ...
            $choice_options = [];
            foreach (json_decode($data['choice_options'], true) as $key => $choice) {
                if(isset($data->unit) && $choice['title'] == $data->unit){
                    $choice_options[$key] = $choice_options && $choice_options[0] ?  $choice_options[0] : null;
                    $choice_options[0] = [
                        'name' => $choice['name'],
                        'title' => $choice['title'],
...
//            $data['choice_options'] = json_decode($data['choice_options']);
    ...
    {
        $x = ['kg', 'pc', 'mg', 'gms', 'ltrs', 'ml', 'pcs', 'bundle', 'pair', 'box', 'carton', 'dozen', 'set'];
        return $x;
    }
...
//            dd($str);
    ...
    public static function gen_mpdf($view, $file_prefix, $file_postfix)
    {
        ...
        $mpdf->Output($file_prefix . $file_postfix . '.pdf', 'D','I');
    }
    public static function gen_product_list($view, $file_prefix, $file_postfix)
    {
        $mpdf = new \Mpdf\Mpdf(['default_font' => 'FreeSerif', 'mode' => 'utf-8', 'format' => [190, 250]]);
        /* $mpdf->AddPage('XL', '', '', '', '', 10, 10, 10, '10', '270', '');*/
        $mpdf->autoScriptToLang = true;
    ...
    public static function print_mpdf($view, $file_prefix, $file_postfix)
    {
        $mpdf = new \Mpdf\Mpdf(['default_font' => 'FreeSerif', 'mode' => 'utf-8', 'format' => [190, 250]]);
        /* $mpdf->AddPage('XL', '', '', '', '', 10, 10, 10, '10', '270', '');*/
        $mpdf->autoScriptToLang = true;
    ...
    public static function affiliate_session_exists()
    {
        return session()->exists('bppshop_affiliate_login');
    }

    public static function affiliate_session_create($agent_id)
    {
        if (!\session()->exists('bppshop_affiliate_login')){
            \session()->put("bppshop_affiliate_login", $agent_id);
        }
        return true;
    }

    public static function affiliate_session_destroy($agent_id)
    {
        if (\session()->exists('bppshop_affiliate_login')){
            \session()->forget('bppshop_affiliate_login');
        }
        return true;
    }

    public static function affiliate_agent_id()
    {
        return \session()->get('bppshop_affiliate_login');
    }
}
...
function checkOTPLimit($mobile, $scope){
    $otpLimit = OTP::where('mobile', $mobile)
        ->where('scope', $scope)
...
function generateOTP($mobile, $scope, $is_resend = 0)
{
...
function checkOTP(OTP $otp, $password)
{
    if ($otp->password != $password){
        OTP::where('id',$otp->id)->update(['is_expired' => 1]);
        return 'invalid';
    }

    $validity = 5;

    $minutes = (time() - strtotime($otp->expired_at)) / 60;
    if ($otp->is_expired || $minutes > $validity){
        OTP::where('id',$otp->id)->update(['is_expired' => 1]);
        return 'expired';
    }
    OTP::where('id',$otp->id)->update(['is_expired' => 1]);
